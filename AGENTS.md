...
- `gomut ./sample` のような位置引数で単一パッケージを対象化できる
- `gomut ./...` でリポジトリ内の Go パッケージを対象化できる
- `diff` サブコマンドで git 差分に含まれる対象を走査できる
...
- 変更後は、必要なら `go run ./cmd/gomut ./sample --jsonl` や `./gomut ./...` で挙動確認する
